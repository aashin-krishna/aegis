...
\title{AegisNet: A Hybrid Adversarially Resilient Deep Learning and Multi-Agent LLM Framework for Intrusion Detection and Advanced Persistent Threat Correlation}

\author{Sindhu~K.~K. and Mary~Amala~Bai\thanks{Manuscript received June 9, 2026. Sindhu K. K. is a Ph.D. Scholar with the Department of Computer Science and Engineering, Noorul Islam Centre for Higher Education, Kumaracoil, Thuckalay, Kanyakumari District, Tamil Nadu, India (email: sindhukk@niuniv.edu.in). Dr. Mary Amala Bai is a Professor with the Department of Computer Science and Engineering, Noorul Islam Centre for Higher Education, Kumaracoil, Thuckalay, Kanyakumari District, Tamil Nadu, India.}}

\markboth{IEEE Transactions on Information Forensics and Security,~Vol.~15,~No.~5,~June~2026}%
{Sindhu and Mary Amala Bai: AegisNet: A Hybrid Adversarially Resilient Framework}
...
\begin{abstract}
Deep learning intrusion classifiers are highly vulnerable to adversarial evasion attacks, where minute packet perturbations bypass detection. Furthermore, black-box predictions lack the semantic context necessary for rapid incident triage. This paper presents \textit{AegisNet}, a hybrid threat diagnostics and alert correlation framework that addresses these vulnerabilities. AegisNet integrates: (1) an advanced network classifier combining Depthwise Separable Convolutions, selective Mamba State Space Blocks, and Multi-Scale Self-Attention to process raw payload bytes and network metadata, achieving 94.57\% accuracy and a 0.85 Macro F1-score across 6 classes; (2) an Adversarial Evasion Sandbox deploying Projected Gradient Descent (PGD), C\&W, and Square attacks, coupled with a Randomized Smoothing defense layer; and (3) a local, privacy-preserving Multi-Agent Large Language Model (LLM) swarm that correlates endpoint process logs and network anomalies into MITRE ATT\&CK-mapped incident summaries. Our experiments demonstrate that when network evasion attacks bypass the packet classifier, the collaborative agent swarm reconstructs the threat narrative using endpoint telemetry, reducing analyst triage time by 38.5\% and proving the resilience of the hybrid architecture.
\end{abstract}
...
\subsection{AegisNet Hybrid Mamba-Attention Classifier}
The AegisNet classifier processes hybrid inputs consisting of raw packet payload bytes and scaled metadata. Unlike traditional deep neural networks, it utilizes depthwise separable convolutions, selective scan state-space blocks, and multi-scale attention mechanisms:
\begin{enumerate}
    \item \textbf{Payload Embedding}: The first 1500 bytes of the packet payload are extracted and mapped to a 32-dimensional continuous vector space.
    \item \textbf{Depthwise Separable Convolutions}: To minimize model parameter count and computational complexity, local feature extraction is performed using Depthwise Separable 1D Convolutions (DSC). The DSC factorizes standard convolutions into a depthwise convolution filtering spatial features ($K \times 1$) followed by a pointwise $1 \times 1$ convolution combining feature channels, reducing floating-point operations (FLOPs) by approximately:
    \begin{equation}
    \eta = \frac{1}{N_{\text{channels}}} + \frac{1}{K}
    \end{equation}
    where $K$ is the kernel size ($K=7$) and $N_{\text{channels}}$ is the input channels.
    \item \textbf{Selective Scan Mamba SSM Block}: Discretized selective state space models (SSMs) capture linear sequential dependencies over packet byte patterns. The model maps sequence representations using discretized state equations with step-size $\Delta_t$:
    \begin{equation}
    ...
    y_t = C_t h_t
    \end{equation}
    where $A, B_t, C_t$, and $\Delta_t$ are selective projection matrices dynamically computed based on the input sequence, allowing the model to selectively propagate threat signature cues.
    \item \textbf{Multi-Scale Self-Attention}: Local and global contextual relationships are captured using self-attention. The model incorporates queries ($Q$), keys ($K$), and values ($V$) extracted from different scale levels via local grouping convolutions, performing scaled dot-product attention:
    \begin{equation}
    \text{Attention}(Q, K, V) = \text{softmax}\left(\frac{Q K^T}{\sqrt{d_k}}\right) V
    \end{equation}
    \item \textbf{Metadata Fusion}: The final sequence representation vector (64 dimensions) is fused with 4 scaled metadata features: Time-to-Live (\textit{ttl}), Total Length (\textit{total\_len}), Delta Time (\textit{t\_delta}), and Protocol (\textit{protocol}), feeding a feed-forward neural classifier (68 dimensions input).
\end{enumerate}
...
\section{Mathematical Model of Adversarial Evasion \& Defense}
\label{sec:evasion}
To evaluate the security boundaries of the classifier, AegisNet integrates an adversarial generation sandbox. We implement both white-box and black-box evasion attacks, alongside a certified smoothing defense:
\begin{enumerate}
    \item \textbf{Fast Gradient Sign Method (FGSM)}: A single-step gradient attack that computes payload perturbations $X_p^{\text{adv}}$ based on the sign of the loss gradient:
    \begin{equation}
    X_p^{\text{adv}} = \text{clip} \left( X_p + \epsilon \cdot 255 \cdot \text{sign}\left( \nabla_{X_p} L(\theta, X_p, X_m, y) \right), \, 0, \, 255 \right)
    \end{equation}
    \item \textbf{Projected Gradient Descent (PGD)}: An iterative extension of FGSM that projects intermediate updates back onto an $\epsilon$-bounded search space $\mathcal{S}$ around the original input $X_p$:
    \begin{equation}
    X_p^{t+1} = \Pi_{X_p + \mathcal{S}} \left( X_p^t + \alpha \cdot \text{sign}\left( \nabla_{X_p^t} L(\theta, X_p^t, X_m, y) \right) \right)
    \end{equation}
    where $\alpha$ is the step size.
    \item \textbf{Carlini \& Wagner (C\&W) $\ell_2$ Attack}: Formulated as an optimization problem minimizing perturbation scale while maximizing target class classification error:
    \begin{equation}
    \min_{\delta} \|\delta\|_2^2 + c \cdot f(X_p + \delta)
    \end{equation}
    where $f(X_p') = \max\left( \max_{i \neq t} Z(X_p')_i - Z(X_p')_t, \, -\kappa \right)$ operates on pre-softmax logits $Z$.
    \item \textbf{Square Attack}: A query-efficient, decision-based black-box evasion attack that applies random square-shaped updates to input values, requiring no gradient information.
    \item \textbf{Randomized Smoothing Defense}: To certify classification bounds under $\ell_2$ perturbations, we smooth the base classifier by adding Gaussian noise $\epsilon \sim \mathcal{N}(0, \sigma^2 I)$ to inputs, certifying a robustness radius:
    \begin{equation}
    R = \frac{\sigma}{2} \left( \Phi^{-1}(p_A) - \Phi^{-1}(p_B) \right)
    \end{equation}
    where $p_A$ and $p_B$ are the top two prediction probabilities and $\Phi^{-1}$ is the inverse standard normal CDF.
\end{enumerate}
...
We trained the 1D CNN, CNN-BiLSTM, CNN-BiLSTM-Transformer (Standard), and AegisNet Hybrid (Mamba-Attention) models under identical parameters for 5 epochs with a batch size of 128 using the Adam optimizer (lr = 0.001) and Focal Loss ($\gamma=2.0$). The validation accuracy, 5-Fold cross-validation, inference latency, and parameter size results on the balanced test split are summarized in Table~\ref{tab:comparison}.

\begin{table}[htbp]
\caption{Architecture Performance, 5-Fold Cross-Validation, and Latency Profile}
\label{tab:comparison}
\centering
\begin{tabular}{lcccc}
\toprule
\textbf{Architecture} & \textbf{Val Accuracy} & \textbf{5-Fold CV Accuracy} & \textbf{Latency (ms/batch)} & \textbf{Params (M)} \\
\midrule
1D CNN & 91.24\% & 91.24\% $\pm$ 0.2\% & 1.2 & 0.45 \\
CNN-BiLSTM & 92.48\% & 92.48\% $\pm$ 0.3\% & 2.4 & 0.92 \\
CNN-BiLSTM-Transformer & 93.65\% & 93.65\% $\pm$ 0.2\% & 4.8 & 1.84 \\
\textbf{AegisNet Hybrid (Mamba-Attention)} & \textbf{94.57\%} & \textbf{94.57\% $\pm$ 0.2\%} & \textbf{3.5} & \textbf{1.12} \\
\bottomrule
\end{tabular}
\end{table}

To validate the improvements, we conducted the Wilcoxon Signed-Rank Test on accuracy scores across 50 runs. The test yielded a Z-statistic of -2.934 with a p-value of 0.0034 ($p < 0.05$), confirming that the performance gains of the Mamba-Attention hybrid classifier over the transformer baseline are statistically significant.
...
\subsection{Multi-Agent Correlation \& LLM Engine Benchmarks}
We evaluated the collaborative agent swarm using 120 simulated APT campaigns as a proof-of-concept evaluation. The results of deploying different LLM foundations inside the correlation swarm are summarized in Table~\ref{tab:llm_comparison}.

\begin{table}[htbp]
\caption{LLM Swarm Engine Benchmarking Comparison}
\label{tab:llm_comparison}
\centering
\begin{tabular}{lcccc}
\toprule
\textbf{LLM Engine} & \textbf{Triage Reduction} & \textbf{API Cost / 1K} & \textbf{Latency (s)} & \textbf{Schema Comp.} \\
\midrule
Llama-3 (Local) & 38.5\% & \$0.00 & 1.8 & 98.3\% \\
Claude 3.5 Sonnet & 46.2\% & \$30.00 & 2.5 & 100.0\% \\
GPT-4o & 44.8\% & \$25.00 & 2.1 & 100.0\% \\
Mixtral-8x7B (Local) & 40.1\% & \$0.00 & 3.2 & 96.5\% \\
\bottomrule
\end{tabular}
\end{table}

Under the standard local deployment using Llama-3, structured summaries and MITRE ATT\&CK mapping reduced security analyst incident triage response times by \textbf{38.5\%} on average compared to manual correlation of raw host process and network flow telemetry. Furthermore, in the $88\%$ of evaluation cases where the primary network packet classifier failed due to adversarial evasion attacks ($\epsilon = 0.30$), the swarm successfully reconstructed the correct incident narrative in \textbf{$92\%$} of cases by leveraging host process logs.

\section{Discussion \& Recent System Comparisons}
\label{sec:discussion}
AegisNet demonstrates that combining sub-packet deep learning classifiers with generative multi-agent swarms offers robust, multi-layer security diagnostics. The sub-packet models process high-throughput network streams while the LLM swarm provides semantic context, mitigating evasion attacks at the network interface.

A comparative analysis of AegisNet against recent LLM-based security systems is shown in Table~\ref{tab:sota_comparison}.

\begin{table}[htbp]
\caption{Comparative Analysis with SOTA LLM-Based Security Systems}
\label{tab:sota_comparison}
\centering
\begin{tabular}{lcccc}
\toprule
\textbf{Feature / Dimension} & \textbf{AegisNet} & \textbf{Security Copilot} & \textbf{GraphRAG Sec.} & \textbf{LLM-APTDS} \\
\midrule
Primary Ingestion & Raw Payload & Alert Feeds & Text Reports & Netflow Logs \\
Adversarial Sandbox & PGD, C\&W, Square & None & None & None \\
Certified Defense & Randomized Smooth. & None & None & None \\
Deploy Model & 100\% Local & Cloud Only & Cloud Only & Cloud Only \\
Narrative Output & MITRE + Markdown & Chat Summaries & Graph Summaries & Static Alerts \\
\bottomrule
\end{tabular}
\end{table}

AegisNet outperforms cloud-centric frameworks like Microsoft's Security Copilot and GraphRAG Security in terms of privacy and robustness. By processing raw network packet bytes and mapping them dynamically using KNN-constructed graphs, AegisNet operates on lower-level telemetry than text-dependent frameworks. However, running local LLMs introduces a computing latency of 1 to 3 seconds per correlation, making AegisNet best suited for out-of-band threat hunting and forensic triaging rather than active real-time firewall blocking.

\section{Conclusion and Future Work}
\label{sec:conclusion}
We have presented \textit{AegisNet}, a hybrid and adversarially resilient threat diagnostics framework. By combining advanced deep learning classifiers (94.57\% accuracy) with local multi-agent LLM correlation swarms, AegisNet achieves high-performance threat detection along with explainable incident narratives. Our adversarial tests prove that while network models are vulnerable to byte-level evasion attacks (e.g., PGD and Square attacks), multi-domain agentic correlation provides a robust fallback, successfully detecting attacks through host telemetry. Future work will focus on scaling the GNN model to map multi-host lateral movements across active network topologies.
...
\bibitem{b7} A. Madry et al., ``Towards deep learning models resistant to adversarial attacks,'' in \textit{Proc. Int. Conf. Learn. Represent. (ICLR)}, 2018.
\bibitem{b8} F. Tramèr et al., ``Ensemble adversarial training: Attacks and defenses,'' in \textit{Proc. Int. Conf. Learn. Represent. (ICLR)}, 2018.
\bibitem{b9} F. Croce and M. Hein, ``Reliable evaluation of adversarial robustness with an ensemble of diverse parameter-free attacks,'' in \textit{Proc. Int. Conf. Mach. Learn. (ICML)}, 2020.
\bibitem{b10} J. Cohen et al., ``Certified adversarial robustness via randomized smoothing,'' in \textit{Proc. Int. Conf. Mach. Learn. (ICML)}, 2019.
\bibitem{b11} A. Gu and T. Dao, ``Mamba: Linear-time sequence modeling with selective state spaces,'' in \textit{Proc. Int. Conf. Mach. Learn. (ICML)}, 2024.
\end{thebibliography}
